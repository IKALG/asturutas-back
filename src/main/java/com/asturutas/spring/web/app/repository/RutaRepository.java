...
public interface RutaRepository extends JpaRepository<RutaEntity, Integer> {
	...
	@Query("SELECT r FROM RutaEntity r WHERE upper(r.usuario) = upper(?1)")
	List<RutaEntity> findRutasByUsuario(String usuario);
	
	@Query("SELECT r FROM RutaEntity r WHERE r.movilidadReducida = true")
	List<RutaEntity> findRutasByMovilidad();
	
	@Query("SELECT r FROM RutaEntity r WHERE r.aptoPerros = true")
	List<RutaEntity> findRutasByPerros();
	
	@Query("SELECT r FROM RutaEntity r WHERE r.aptoNinos = true")
	List<RutaEntity> findRutasByNinos();
